...
class Boid(object):
    ...
    def run(self, boids, sep_fac, ali_fac, coh_fac):
        self.flock(boids, sep_fac, ali_fac, coh_fac)
        self.update()
        self.borders()
        self.render()
    ...
    def flock(self, boids, sep_fac, ali_fac, coh_fac):
    ...
    def render(self):
        ...
        if self.polarity < 0:
            col = color((255 + (255 * self.polarity)),
                    (255 + (255 * self.polarity)),
                    255 )
    ...
    def align(self, boids):
        ...
        for other in boids:
            d = PVector.dist(self.location, other.location)
            if 0 < d < neighbordist:
                sum.add(self.polarity * other.polarity  *  other.velocity)
                count += 1
        if count == 0:
            return PVector(0, 0)
        sum.div(float(count))
